  ...
  return (
    <div>
      ...
      {diameter != null && !(DIAMS as readonly number[]).includes(diameter) && (
        <p className="mb-4 rounded-md border border-[#C9A22733] bg-[#C9A22712] px-4 py-2.5 font-mono text-xs text-[#C9A227]">
      ...
      <div className="scroll-slim grid max-h-[420px] grid-cols-2 gap-3 overflow-y-auto pr-1 sm:grid-cols-3">
        {DIAMS.map((d) => {
          ...
          return (
            ...
            >
              <span>
                <span className="block font-display text-xl font-semibold text-[#E8D5A3]">
                  {d} <span className="font-sans text-xs font-normal text-[#C9A22799]">mm</span>
                </span>
                <span className="text-xs text-[#C9A22799]">
                  Taux {fmt2(getDiamRate(d))} DH/cm²
                </span>
              </span>
              {selected && <span className="font-semibold text-[#E8C547]">✓</span>}
            </button>
          );
        })}
      </div>
